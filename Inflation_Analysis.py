...
if __name__=='__main__':
    ...
    #Limit to all-group averages
    totals = df[(df['demographics_code']=='LB01')&(df['characteristics_code']=='04')]

    # totals = pd.DataFrame(df[(df['demographics_code']=='LB01')&(df['characteristics_code'].isin(['03','04','05']))].groupby(by=['year','item_code'])['value'].mean()).reset_index()

    # totals18 = totals[totals.year=='2018'][['series_title','category_code','subcategory_code','value']]
    # sum18 = totals18[totals18.category_code=='EXPEND']['value'].sum()

    years = range(2005,2019)
    exp_vars = ['HOUSING','TRANS','FOODTOTL','HEALTH']
    total_exp = 'TOTALEXP'
    total_inc = 'INCBEFTX'

    cost_rates, cost_list, year_dict = calc_cost_rates(totals, years, exp_vars, total_exp)

    acs_inc_pickle = 'acs_inc.pickle'
    print("...loading pickle")
    tmp = open(filepath+'intermediate_files/'+acs_inc_pickle,'rb')
    dfh_inc = pickle.load(tmp)
    tmp.close()

    dfh_inc = dfh_inc.rename(columns={'YEAR':'year'})
    # yrs = range(2005,2019)

    inc_rates, inc_list  = calc_inc_rates(dfh_inc, years, ['HINCP'])

    #Inelastic goods purchases as percent of median household income over time
    inc_perc = [cost_list[i]/inc_list[i] for i in range(0,14)]

    #Difference between mean and median over time - its EXPLODED!
    diff = median_mean(dfh_inc,years)

    dfh18 = dfh_inc[(dfh_inc.year==2018)&(dfh_inc.HINCP>=0)]['HINCP'].values
    gini18 = gini(dfh18)
    print(gini18)

    dfh05 = dfh_inc[(dfh_inc.year==2005)&(dfh_inc.HINCP>=0)]['HINCP'].values
    gini05 = gini(dfh05)
    print(gini05)
